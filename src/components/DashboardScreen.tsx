...
import { SHAMSI_MONTHS, getTodayShamsi, getDaysInShamsiMonth } from "../utils/shamsi";
...
interface DashboardScreenProps {
  ...
  dashboardPeriod: number | { label: string; startDate: string; endDate: string; days: number };
  setDashboardPeriod: (period: number | { label: string; startDate: string; endDate: string; days: number }) => void;
...
export function DashboardScreen({
  ...
  const currentYear = getTodayShamsi().year;
  const monthOptions: Array<{ label: string; startDate: string; endDate: string; days: number }> = SHAMSI_MONTHS.map((m) => {
    const daysCount = getDaysInShamsiMonth(currentYear, m.id);
    const startDate = `${currentYear}/${m.id.toString().padStart(2, '0')}/01`;
    const endDate = `${currentYear}/${m.id.toString().padStart(2, '0')}/${daysCount.toString().padStart(2, '0')}`;
    return {
      label: `ماه ${m.name}`,
      startDate,
      endDate,
  ...
  return (
    ...
    >
          ...
          <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row items-center gap-4 justify-between" dir="rtl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    typeof dashboardPeriod === 'number' && dashboardPeriod === option.value
              ...
              <div className="flex items-center gap-2 mr-2 border-r border-slate-200 pr-4">
                <span className="text-xs text-slate-500 font-bold">انتخاب ماه:</span>
                <select
                  onChange={(e) => {
                    const selected = monthOptions.find(o => o.startDate === e.target.value);
                    if (selected) {
                      setDashboardPeriod(selected);
                    }
                  }}
                  value={typeof dashboardPeriod === 'object' ? dashboardPeriod.startDate : ""}
                  className="px-2 py-1.5 border border-slate-300 rounded-lg text-xs font-bold bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="" disabled>انتخاب ماه خاص...</option>
                  {monthOptions.map(opt => (
                    <option key={opt.startDate} value={opt.startDate}>
                      {opt.label}
                    </option>
                  ))}
                </select>
              ...
              <div className="flex items-center gap-2 mr-2 border-r border-slate-200 pr-4">
                ...
                <input
                  type="number"
                  min="1"
                  value={typeof dashboardPeriod === 'number' ? dashboardPeriod : ""}
                  placeholder="روز"
                  onChange={(e) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-500 font-bold block mb-1">حجم کل فروش {dashboardPeriodLabel}</span>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-500 font-bold block mb-1">حجم کل ورودی {dashboardPeriodLabel}</span>
            <span className="text-2xl font-extrabold font-mono text-emerald-600">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-500 font-bold block mb-1">برترین رانندگان ({dashboardPeriodLabel})</span>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-500 font-bold block mb-1">تعداد کل سفارشات مشتریان ({dashboardPeriodLabel})</span>
            <span className="text-2xl font-extrabold font-mono text-cyan-600">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-3 mb-4 gap-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800">
                  ۱۰ محصول پرفروش انبار ({dashboardPeriodLabel})
                </h3>
              </div>
