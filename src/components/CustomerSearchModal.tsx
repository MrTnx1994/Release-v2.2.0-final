...
export function CustomerSearchModal({
  ...
  const [selectedPreset, setSelectedPreset] = useState<"30days" | "thisYear" | "pastYear" | "lastYear" | "allTime" | "today" | "tomorrow">("30days");
  ...
  const computeDates = () => {
    ...
    if (activeFilterType === "preset") {
      if (selectedPreset === "30days") {
        ...
        from = today.formatted;
        to = today.formatted;
        label = `امروز (${from})`;
      } else if (selectedPreset === "tomorrow") {
        const jsDate = new Date();
        jsDate.setDate(jsDate.getDate() + 1);
        const shamsiTom = gregorianToShamsi(jsDate.getFullYear(), jsDate.getMonth() + 1, jsDate.getDate());
        from = shamsiTom.formatted;
        to = shamsiTom.formatted;
        label = `فردا (${from})`;
          ...
          <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100">
            {activeFilterType === "preset" && (
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {[
                  { id: "today", name: "ارسالی امروز" },
                  { id: "tomorrow", name: "ارسالی فردا" },
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span className="font-mono text-indigo-700 font-black bg-indigo-50/60 px-1.5 py-0.5 rounded-md text-[11px]">
                                    {qty} <span className="text-[9px] font-medium font-sans">کیلوگرم</span>
                                  </span>
                                </div>
                              );
                            })
