...
export function AppHeader({ user, role, logout, isDarkMode, setIsDarkMode, todayShamsi, shamsiYear, shamsiMonth, shamsiDay, formattedDate, currentDateHolidayInfo, handlePrevDay, handleNextDay, handleJumpNextWorkingDay, handleJumpPrevWorkingDay, handleJumpToday, setShowDatePicker, setTempYear, setTempMonth, setTempDay, setManualDateInput, activeTab, setActiveTab, onOpenChangePassword }: AppHeaderProps) {
  ...
  return <>
    ...
    <nav className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 overflow-x-auto flex justify-start items-center gap-1.5 no-print shrink-0 py-2 sm:py-2.5">{navItems.map(([key, Icon, label, visible]) => visible && <button key={key} onClick={() => setActiveTab(key)} className={`flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-bold transition shrink-0 cursor-pointer ${activeTab === key ? "bg-cyan-50 dark:bg-cyan-950/50 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800 shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800/60"}`}><Icon className="w-4 h-4" />{label}</button>)}</nav>
  </>;
}
