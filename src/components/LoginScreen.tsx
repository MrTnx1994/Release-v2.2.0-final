...
import { KeyRound, User, AlertCircle, LogIn, Truck, Calendar, Sun, Moon, Eye, EyeOff, ShieldCheck, TrendingUp, MapPin, LineChart, PhoneCall, Copy, Check, Sparkles } from 'lucide-react';
...
export const LoginScreen = () => {
  ...
  const features = [
    ...
    { icon: LineChart, text: "گزارش کلی انبار و پیش بینی تولید" },
  ];
  ...
  return (
    <div className="relative flex flex-col min-h-screen font-sans transition-colors duration-300" dir="rtl">
      ...
      <div className="fixed inset-0 bg-white/30 dark:bg-slate-950/40 backdrop-blur-sm" />
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center p-4 w-full">
        ...
        >
          ...
          <div className="relative hidden md:flex flex-col p-9 text-white overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-blue-600/85 via-indigo-700/85 to-violet-800/85" />
            <div className="absolute -top-16 -left-16 w-64 h-64 bg-cyan-400/20 rounded-full blur-3xl" />
            <div className="absolute -bottom-24 -right-10 w-72 h-72 bg-fuchsia-400/20 rounded-full blur-3xl" />
            <div className="absolute top-1/3 left-1/2 w-56 h-56 bg-amber-300/15 rounded-full blur-3xl" />
              ...
              <div className="mt-8 space-y-3">
                {features.map((f, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 bg-white/10 rounded-xl px-3.5 py-2.5 backdrop-blur-sm border border-white/20 shadow-sm"
                  >
                    <f.icon className="w-4 h-4 text-blue-200 shrink-0" />
                    <span className="text-xs font-semibold text-white/95">{f.text}</span>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative group">
                  <span className="absolute inset-y-0 right-0 flex items-center pr-3.5 pointer-events-none text-slate-400 dark:text-slate-500 group-focus-within:text-blue-600 dark:group-focus-within:text-blue-400 transition-colors">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full pl-3 pr-10 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500 focus:bg-white dark:focus:bg-slate-900 transition-all"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500 focus:bg-white dark:focus:bg-slate-900 transition-all"
                  />
                  <button
                    type="button"
              ...
              <motion.button
                ...
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-gradient-to-l from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed mt-2"
              >
                {loading ? (
                  <>
      ...
      <footer className="relative z-10 py-5 px-6 border-t border-white/10 bg-black/20 backdrop-blur-md text-center text-xs text-white/70 select-none no-print transition-colors duration-300 w-full">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          {/* شماره تماس پشتیبانی و داخلی با قابلیت تماس مستقیم و کپی */}
          <div className="flex items-center gap-2 bg-white/10 hover:bg-white/15 px-3.5 py-1.5 rounded-full border border-white/15 backdrop-blur-md transition-all shadow-sm">
            <a
              href="tel:02137832,502"
              className="flex items-center gap-2 text-white/90 hover:text-cyan-300 transition-colors font-medium cursor-pointer"
              title="برای برقراری تماس مستقیم کلیک کنید"
            >
              <PhoneCall className="w-3.5 h-3.5 text-emerald-400 animate-pulse shrink-0" />
              <span>شماره تماس پشتیبانی :</span>
              <span className="font-mono font-bold text-cyan-200 tracking-wider dir-ltr inline-block">02137832</span>
              <span className="text-white/40">|</span>
              <span>داخلی:</span>
              <span className="font-mono font-bold text-amber-300">502</span>
            </a>

            <button
              type="button"
              onClick={handleCopyPhone}
              className="mr-1 p-1 hover:bg-white/20 rounded-md text-white/70 hover:text-white transition-all cursor-pointer flex items-center gap-1 text-[11px]"
            ...
            >
              {copiedPhone ? (
                ...
                <Copy className="w-3 h-3 hover:text-cyan-200" />
              )}
            </button>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 bg-white/15 hover:bg-white/25 px-3 py-1.5 rounded-full border border-white/20 font-mono text-[11px] font-bold text-cyan-200 hover:text-white transition-all cursor-pointer shadow-xs"
              title="مشاهده تاریخچه تغییرات و نگارش‌های سامانه"
            >
              <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse"></span>
              <span>نسخه: v{APP_VERSION}</span>
            </button>

            <div className="hidden sm:flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full border border-white/15 font-mono text-[11px] font-semibold text-white/80 hover:text-cyan-300 transition-all">
