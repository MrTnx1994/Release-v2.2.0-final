...
export const StockForecastScreen: React.FC<StockForecastScreenProps> = ({
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (active) {
            const loadedCoverage = data.coverageDays ?? 15;
            const loadedSafety = data.safetyDays ?? 3;
            const loadedAverage = data.averageDays ?? 30;
            const loadedHidden = data.hiddenProductIds ?? [];

            lastSavedSettingsRef.current = JSON.stringify({
              coverageDays: loadedCoverage,
              safetyDays: loadedSafety,
              averageDays: loadedAverage,
              hiddenProductIds: loadedHidden,
            });

            setCoverageDays(loadedCoverage);
            setSafetyDays(loadedSafety);
            setAverageDays(loadedAverage);
            setHiddenProductIds(loadedHidden);
            setIsLoaded(true);
          }
        } else {
          if (active) {
  ...
  // Debounced save forecast settings to server - only when actually changed
  useEffect(() => {
    if (!isLoaded) return;
    ...
    // Skip saving if nothing changed compared to what was loaded or last saved
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        lastSavedSettingsRef.current = currentStr;
      } catch (err) {
