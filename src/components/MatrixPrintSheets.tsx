                    ...
                    invoices
                      ...
                      .forEach((inv) => {
                        filteredProducts.forEach((p) => {
                          totalWeight += Number(inv.quantities[p.id] || 0);
                        });
                      });
                  }
...
export function renderDriverSheetsContent(
  ...
  return (
    <div className={`font-sans ${isPrint ? "space-y-0 print:space-y-0" : "space-y-8"}`} dir="rtl">
      {driversToRender.map((drvName, idx) => {
        ...
        return (
          <div 
            key={drvName} 
            className={`driver-sheet-page bg-white text-black p-6 border border-slate-300 rounded-2xl shadow-sm ${
              isPrint ? "print:border-0 print:p-0 print:shadow-none print:m-0" : ""
            } relative overflow-hidden`}
            style={
              isPrint
                ? {
                    pageBreakAfter: idx < driversToRender.length - 1 ? "always" : "auto",
                    breakAfter: idx < driversToRender.length - 1 ? "page" : "auto",
