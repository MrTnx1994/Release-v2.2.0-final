...
import { getTodayShamsi } from "./src/utils/shamsi";
...
async function startServer() {
    ...
    const setupTables = (conn: DatabaseSync) => {
      ...
      try {
        conn.exec(`
          ...
          SET userId = 'admin@system.com' 
          WHERE userId = 'unknown@system.com' 
             OR userId = 'unknown' 
             OR userId LIKE '%unknown%' 
             OR userId IS NULL 
             OR userId = '';
          UPDATE activity_logs 
          SET details = REPLACE(details, 'unknown@system.com', 'admin@system.com') 
          WHERE details LIKE '%unknown@system.com%';
          UPDATE activity_logs 
          SET details = REPLACE(details, 'کاربر unknown', 'کاربر admin@system.com') 
          WHERE details LIKE '%کاربر unknown%';
        `);
      } catch {}
    };
  ...
  const stmtGetHoliday = db.prepare("SELECT * FROM holidays WHERE date = ?");
  const stmtInsertHoliday = db.prepare("INSERT OR REPLACE INTO holidays (date, title) VALUES (?, ?)");
  const stmtGetAllHolidays = db.prepare("SELECT * FROM holidays");

  // Sync Holidays logic
  const syncHolidaysForYear = async (year: number) => {
    try {
      const existingHolidays = stmtGetAllHolidays.all() as { date: string, title: string }[];
      const yearStr = String(year);
      const hasYear = existingHolidays.some(h => h.date.startsWith(yearStr + '/'));
      
      if (!hasYear) {
        console.log(`[Holidays] Fetching holidays for year ${year}...`);
        const res = await fetch(`https://pnldev.com/api/calender?year=${year}`);
        if (!res.ok) throw new Error("Failed to fetch holidays from pnldev API");
        const data = await res.json();
        
        if (data && data.status && data.result) {
          let count = 0;
          for (const [monthNum, daysObj] of Object.entries(data.result)) {
            for (const [dayNum, dayInfo] of Object.entries(daysObj as any)) {
              if (dayInfo && (dayInfo as any).holiday === true) {
                const dateStr = `${year}/${String(monthNum).padStart(2, '0')}/${String(dayNum).padStart(2, '0')}`;
                let title = ((dayInfo as any).event || []).join(' / ');
                if (!title) title = "تعطیل رسمی";
                stmtInsertHoliday.run(dateStr, title);
                count++;
              }
            }
          }
          console.log(`[Holidays] Successfully synced ${count} holidays for year ${year}.`);
        }
      }
    } catch (err) {
      console.error(`[Holidays] Error syncing holidays for year ${year}:`, err);
    }
  };

  // Run holiday sync on startup
  setTimeout(() => {
    const today = getTodayShamsi();
    syncHolidaysForYear(today.year);
    syncHolidaysForYear(today.year + 1); // Also pre-fetch next year
  }, 5000);
  ...
  const stmtDailyPlansBeforeDesc = db.prepare("SELECT date, data FROM daily_plans WHERE date < ? ORDER BY date DESC LIMIT ?");
  const stmtDailyPlansUpToDesc = db.prepare("SELECT date, data FROM daily_plans WHERE date <= ? ORDER BY date DESC LIMIT ?");
  const stmtDailyPlansAllDesc = db.prepare("SELECT date, data FROM daily_plans ORDER BY date DESC LIMIT ?");
  ...
  const stmtDeleteAllUsersExcept = db.prepare("DELETE FROM users WHERE email NOT IN ('admin@system.com', 'nazari925@gmail.com')");
      ...
      try {
        logs.forEach((log: any) => {
          stmtInsertLog.run(
            ...
            String((log.userId && log.userId !== "unknown@system.com" && log.userId !== "unknown") ? log.userId : "admin@system.com"),
            String(log.action ?? ""),
  ...
  const userEmailToSeed = "nazari925@gmail.com";
  const userExists = stmtGetUserByEmail.get(userEmailToSeed);
  if (!userExists) {
    stmtInsertUser.run(
      "user_seed_uid",
      userEmailToSeed,
    ...
    console.log(`Seeded default account (${userEmailToSeed} / 010203) — please change this password after first login.`);
  }
  ...
  const PRIMARY_ADMIN_EMAILS = ["admin@system.com", "nazari925@gmail.com"];
  const requirePrimaryAdmin = (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const user = (req as any).user;
    if (!user || !PRIMARY_ADMIN_EMAILS.includes(user.email.toLowerCase())) {
  ...
  app.get("/api/forecast-settings", requireAuth, (req, res) => {
    try {
      const row = stmtGetConfig.get("forecast_settings") as { value: string } | undefined;
      if (row) {
  ...
  app.post("/api/forecast-settings", requireAuth, (req, res) => {
    try {
      const { coverageDays, safetyDays, averageDays, hiddenProductIds } = req.body;
      const settings = {
        coverageDays: Number(coverageDays) || 15,
        safetyDays: Number(safetyDays) || 3,
        averageDays: Number(averageDays) || 30,
        hiddenProductIds: Array.isArray(hiddenProductIds) ? hiddenProductIds : []
      };
      stmtSetConfig.run("forecast_settings", JSON.stringify(settings));

      stmtInsertLog.run(
        Date.now().toString() + Math.random().toString(36).slice(2, 6),
        (req as any).user.email,
        "تغییر تنظیمات پیش‌بینی انبار",
        `تنظیمات پیش‌بینی انبار توسط کاربر به دوره پوشش: ${settings.coverageDays}، حاشیه امنیت: ${settings.safetyDays}، میانگین‌گیری: ${settings.averageDays} و تعداد کالاهای پنهان: ${settings.hiddenProductIds.length} تغییر یافت.`,
  ...
  app.get("/api/load/:date", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    const { date } = req.params;
  ...
  app.post("/api/save", requireAuth, requireRole("admin", "sales", "driver"), (req, res) => {
    try {
      ...
      const email = authedUser?.email || (userEmail && userEmail !== "unknown@system.com" && userEmail !== "unknown" ? userEmail : "admin@system.com");
  ...
  app.get("/api/statistics/sales", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    try {
      const { endDate, days, startDate } = req.query as { endDate?: string; days?: string; startDate?: string };
      const salesMap: { [productId: string]: number } = {};
      const products = getConfigValue("products") || [];
      if (Array.isArray(products)) {
        products.forEach((p: any) => { if (p && p.id) salesMap[p.id] = 0; });
      }

      let rows: { date: string; data: string }[] = [];
      if (startDate && endDate) {
        rows = stmtGetDailyPlansInRange.all(startDate, endDate) as { date: string; data: string }[];
      } else {
        const limitDays = days ? parseInt(days, 10) : 30;
        rows = (endDate ? stmtDailyPlansBeforeDesc.all(endDate, limitDays) : stmtDailyPlansAllDesc.all(limitDays)) as { date: string; data: string }[];
      }
      rows.forEach((row) => {
        try {
          if (!row || !row.data) return;
          const dayData = JSON.parse(row.data);
          if (dayData && Array.isArray(dayData.invoices)) {
            dayData.invoices.forEach((inv: any) => {
              if (inv && inv.isActive !== false && inv.quantities) {
                Object.entries(inv.quantities).forEach(([pId, qty]) => {
          ...
          console.error("Error parsing daily plan JSON in sales stats:", e);
        }
      });

      res.json({ sales: salesMap });
    } catch (err: any) {
      console.error("Error in /api/statistics/sales:", err);
      res.status(500).json({ status: "error", message: err.message || "خطا در دریافت آمار فروش" });
    }
  });

  app.get("/api/statistics/incoming", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    try {
      const { endDate, days, startDate } = req.query as { endDate?: string; days?: string; startDate?: string };
      const incomingMap: { [productId: string]: number } = {};
      const products = getConfigValue("products") || [];
      if (Array.isArray(products)) {
        products.forEach((p: any) => { if (p && p.id) incomingMap[p.id] = 0; });
      }
      ...
      rows.forEach((row) => {
        try {
          if (!row || !row.data) return;
          const dayData = JSON.parse(row.data);
          if (dayData && dayData.manualStockOverrides && typeof dayData.manualStockOverrides === "object") {
          ...
          console.error("Error parsing daily plan JSON in incoming stats:", e);
        }
      });

      res.json({ incoming: incomingMap });
    } catch (err: any) {
      console.error("Error in /api/statistics/incoming:", err);
      res.status(500).json({ status: "error", message: err.message || "خطا در دریافت آمار ورودی" });
    }
  });

  app.get("/api/statistics/invoices", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    try {
      const { endDate, days, startDate } = req.query as { endDate?: string; days?: string; startDate?: string };
      ...
      if (startDate && endDate) {
        ...
        const limitDays = days ? parseInt(days, 10) : 30;
        rows = (endDate ? stmtDailyPlansUpToDesc.all(endDate, limitDays) : stmtDailyPlansAllDesc.all(limitDays)) as { date: string; data: string }[];
      }
      rows.forEach((row) => {
        try {
          if (!row || !row.data) return;
          ...
          if (dayData && Array.isArray(dayData.invoices)) {
            dayData.invoices.forEach((inv: any) => {
              if (inv && inv.isActive !== false) {
                ...
                if (inv.driverName && typeof inv.driverName === "string") {
          ...
          console.error("Error parsing daily plan JSON in invoices stats:", e);
        }
      });
      res.json({ driverStats, totalOrders });
    } catch (err: any) {
      console.error("Error in /api/statistics/invoices:", err);
      res.status(500).json({ status: "error", message: err.message || "خطا در دریافت آمار فاکتورها" });
    }
  });
  ...
  app.get("/api/search/customer", requireAuth, (req, res) => {
    const { name, fromDate, toDate } = req.query as { name?: string; fromDate?: string; toDate?: string };
    if (!name || !name.trim()) {
      return res.status(400).json({ status: "error", message: "نام مشتری برای جستجو الزامی است." });
    }

    const finalFrom = fromDate || "1300/01/01";
    const finalTo = toDate || "1499/12/29";
    ...
    // Get plans in range
    const rows = stmtGetDailyPlansInRange.all(finalFrom, finalTo) as { date: string; data: string }[];
    // Reverse to have descending order (newest first)
    rows.reverse();

    // Loaded ONCE, outside the loops below
    const productsList = getConfigValue("products");
  ...
  app.get("/api/holidays", (req, res) => {
    try {
      const all = stmtGetAllHolidays.all() as { date: string, title: string }[];
  ...
  app.post("/api/auth/login", loginLimiter, async (req, res) => {
    ...
    if (!passwordOk) {
      console.log(`Login failed for user ${email}: password mismatch`);
  ...
  app.post("/api/users/delete", requireAuth, requireRole("admin"), (req, res) => {
    try {
      ...
      if (PRIMARY_ADMIN_EMAILS.includes(user.email.toLowerCase())) {
  ...
  app.post("/api/logs/create", requireAuth, (req, res) => {
    try {
      ...
      const authedEmail = (req as any).user?.email;
      const effectiveUserId = authedEmail || (userId && userId !== "unknown@system.com" && userId !== "unknown" ? userId : "admin@system.com");
      const newLog = {
  ...
  app.get("/api/backup/download", requireAuth, requireRole("admin"), (req, res) => {
    try {
      ...
      const backup = {
        ...
        dailyPlans: dailyPlanRows.map((r) => {
          let parsedData = {};
          try {
          ...
          return { date: r.date, data: parsedData };
        }),
  ...
  const normalizeBackupPayload = (input: any) => {
    if (!input || typeof input !== "object") {
      return { drivers: [], products: [], users: [], dailyPlans: [], activityLogs: [] };
    }

    const drivers = Array.isArray(input.drivers) ? input.drivers : [];
    const products = Array.isArray(input.products) ? input.products : [];
    const users = Array.isArray(input.users) ? input.users : [];
    const activityLogs = Array.isArray(input.activityLogs) ? input.activityLogs : (Array.isArray(input.activity_logs) ? input.activity_logs : []);

    const dailyPlans: { date: string; data: any }[] = [];

    if (Array.isArray(input.dailyPlans)) {
      input.dailyPlans.forEach((dp: any) => {
        if (dp && dp.date) {
          dailyPlans.push({ date: String(dp.date), data: dp.data || dp });
        }
      });
    } else if (Array.isArray(input.daily_plans)) {
      input.daily_plans.forEach((dp: any) => {
        if (dp && dp.date) {
          dailyPlans.push({ date: String(dp.date), data: dp.data || dp });
        }
      });
    ...
    // Check all top-level keys for date-like keys or date_ prefixed keys
    Object.keys(input).forEach((key) => {
      if (["drivers", "products", "users", "activityLogs", "activity_logs", "range", "dailyPlans", "daily_plans"].includes(key)) return;
      
      let dateStr = key;
      if (key.startsWith("date_")) {
        dateStr = key.replace("date_", "");
      }

      if (dateStr.includes("/") || dateStr.includes("-") || /^\d{4,8}$/.test(dateStr)) {
        const val = input[key];
        if (val && typeof val === "object") {
          if (!dailyPlans.some(p => p.date === dateStr)) {
            dailyPlans.push({ date: dateStr, data: val });
          }
        }
      }
    });
    ...
    try {
      ...
      try {
        if (Array.isArray(backup.drivers)) {
          ...
          const driverMap = new Map(currentDrivers.map((d: any) => [d.name, d]));
          let driversChanged = false;
          backup.drivers.forEach((d: any) => {
            if (d && d.name) {
              if (driverMap.has(d.name)) {
                const existingD = driverMap.get(d.name);
                if (JSON.stringify(existingD) !== JSON.stringify(d)) {
                  driverMap.set(d.name, d);
                  driversChanged = true;
          ...
          if (driversChanged) {
            stmtSetConfig.run("drivers", JSON.stringify(Array.from(driverMap.values())));
          }
        }
        ...
        if (Array.isArray(backup.products)) {
          ...
          backup.products.forEach((p: any) => {
            if (p && p.id) {
              if (productMap.has(p.id)) {
                const existingP = productMap.get(p.id);
                if (JSON.stringify(existingP) !== JSON.stringify(p)) {
          ...
          if (productsChanged) {
            stmtSetConfig.run("products", JSON.stringify(Array.from(productMap.values())));
          }
        }
          ...
          backup.users.forEach((u: any) => {
            ...
            if (!existing) {
              ...
              if (u.role && u.role !== existing.role) {
                db.prepare("UPDATE users SET role = ? WHERE email = ?").run(u.role, u.email);
                summary.usersAdded++;
              }
              if (u.status && u.status !== existing.status) {
                db.prepare("UPDATE users SET status = ? WHERE email = ?").run(u.status, u.email);
              }
              if (u.driverName !== undefined && u.driverName !== existing.driverName) {
                db.prepare("UPDATE users SET driver_name = ? WHERE email = ?").run(u.driverName, u.email);
              }
            }
          });
        }
        ...
        if (Array.isArray(backup.dailyPlans)) {
          backup.dailyPlans.forEach((entry: any) => {
            ...
            if (!existingRow) {
              stmtUpsertDailyPlan.run(entry.date, JSON.stringify(data));
              summary.dailyPlansAdded++;
            } else {
              try {
                const existingData = JSON.parse(existingRow.data);
                existingData.invoices = Array.isArray(existingData.invoices) ? existingData.invoices : [];
                const existingInvoiceMap = new Map();
                existingData.invoices.forEach((inv: any, idx: number) => {
                  if (inv && inv.id) {
                    existingInvoiceMap.set(inv.id, idx);
                  }
                });
                ...
                (data.invoices || []).forEach((inv: any) => {
                  if (inv && inv.id) {
                    if (existingInvoiceMap.has(inv.id)) {
                      const idx = existingInvoiceMap.get(inv.id);
                      const oldInv = existingData.invoices[idx];
                      if (JSON.stringify(oldInv) !== JSON.stringify(inv)) {
                        existingData.invoices[idx] = inv;
                        addedAny = true;
                      }
                    } else {
                      existingData.invoices.push(inv);
                      existingInvoiceMap.set(inv.id, existingData.invoices.length - 1);
                      addedAny = true;
                    }
                  }
                });

                if (data.manualStockOverrides && typeof data.manualStockOverrides === "object") {
                  existingData.manualStockOverrides = existingData.manualStockOverrides || {};
                  Object.entries(data.manualStockOverrides).forEach(([pid, val]) => {
                    if (existingData.manualStockOverrides[pid] !== val) {
        ...
        if (Array.isArray(backup.activityLogs)) {
          backup.activityLogs.forEach((log: any) => {
            ...
            try {
              stmtInsertLog.run(
                log.id,
                (log.userId && log.userId !== "unknown@system.com" && log.userId !== "unknown") ? log.userId : "admin@system.com",
                log.action || "",
  ...
  if (process.env.NODE_ENV === "production") {
    app.listen(3000, () => {
      console.log(`Server running in production on port ${3000}`);
    });
  } else {
